...
p <- ggplot(data = d, aes(x = value)) +
  ...
  labs(x = "", y = "Density")
ggsave("figs/fig-02-01.pdf", width = 8, height = 6)
...
p <- ggplot(data = d, aes(x = sex, y = value)) +
  ...
  labs(x = NULL, y = NULL)
ggsave("figs/fig-02-02.pdf", width = 8, height = 6)
...
cairo_pdf("figs/fig-02-03.pdf", width = 8, height = 6)
plot_grid(p1, p2)
dev.off()
...
ggsave("figs/fig-02-04.pdf", width = 8, height = 6)
...
yhat <- cbind.data.frame(dd,
  y1 = predict(m1, dd), y2 = predict(m2, dd),
...
p <- ggplot(data = FEV, aes(x = height, y = fev)) +
  ...
  scale_color_manual("",
    values = c("#d18975", "#8fd175", "#3f2d54", "#75b8d1"),
    labels = c("height + height²", "pol(height, 3)", "rcs(height, 3)", "fp(height, 4)")
  ) +
  theme(legend.position = c(0.1, 0.88)) +
  labs(x = "Height (cm)", y = "FEV (l/s)")
ggsave("figs/fig-02-06.pdf", width = 8, height = 6)
...
p <- ggplot(data = FEV, aes(x = age, y = fev)) +
  ...
  labs(x = "Age (yr.)", y = "FEV (l/s)")
ggsave("figs/fig-02-07.pdf", width = 8, height = 6)
FEV$age4c <- NULL
...
p <- ggplot(data = FEV, aes(x = age, y = fev, color = smoke)) +
  ...
  guides(color = FALSE) +
  labs(x = "Age (yr.)", y = "FEV (l/s)")
cairo_pdf("figs/fig-02-08.pdf", width = 8, height = 6)
...
p <- ggplot(data = FEV, aes(x = age, y = fev, color = smoke)) +
  ...
  guides(color = FALSE) +
  labs(x = "Age (yr.)", y = "FEV (l/s)")
cairo_pdf("figs/fig-02-09.pdf", width = 8, height = 6)
direct.label(p + aes(label = smoke))
dev.off()
...
d <- expand.grid(
  age = 9:18, smoke = levels(FEV$smoke),
...
Predict(m,
  age = 18, height = seq(160, 180, by = 2),
...
cairo_pdf("figs/fig-02-10.pdf", width = 8, height = 6)
...
ggsave("figs/fig-02-11.pdf", width = 8, height = 6)
